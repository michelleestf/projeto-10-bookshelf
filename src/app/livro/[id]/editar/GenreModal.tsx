  ...
  return (
    <DialogContent onInteractOutside={e => e.preventDefault()}>
      <DialogHeader>
        ...
        <DialogDescription>
          Os gêneros são <b>globais</b> e compartilhados entre todos os livros.<br />
          <b>Adicionar</b> um novo gênero irá incluí-lo no banco de dados e ficará disponível para todos os livros.<br />
          <b>Excluir</b> um gênero remove do banco de dados e de todos os livros que o utilizam.
        </DialogDescription>
      </DialogHeader>
      <Button
        type="button"
        variant="ghost"
        className="absolute top-2 right-2 p-1 h-7 w-7 text-gray-500 hover:text-black"
        onClick={onClose}
            ...
            <ul className="absolute z-10 left-0 right-0 bg-white border rounded shadow mt-1 max-h-32 overflow-auto text-xs top-full">
              {allGenreSuggestions
                .filter((g) =>
                  g
                    .toLowerCase()
                    .includes(newGenre.trim().toLowerCase())
                )
                .map((g) => (
                  <li
                    key={g}
        ...
        <div className="flex flex-wrap gap-2">
          {genres.length > 0 ? (
            genres.map((g) => (
              ...
              >
                <span title="Gênero global, usado em todos os livros">{g}</span>
                <button
                  type="button"
            ...
            <span className="text-xs text-neutral-400">Nenhum gênero cadastrado.</span>
          )}
        </div>
      </div>
    </DialogContent>
